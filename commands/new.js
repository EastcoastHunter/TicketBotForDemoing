...
const { support_team_role, support_manager_role } = require("../storage/config.json");
  ...
  execute: async (message, args) => {
    ...
    if(!message.member.roles.some(r=>[config.support_team_role].includes(r.name)) && !message.member.roles.some(r=>[config.support_manager_role].includes(r.name))) {
      ...
      message.guild.createChannel(`ticket-${tcc}`, 'text').then(async channel => {
        ...
        channel.send(`<@&${support_team_role.id}> , <@&${support_manager_role.id}> ` + message.author , embed);
    ...
    if(member) {
      ...
      message.guild.createChannel(`ticket-${tcc}`, 'text').then(async channel => {
        ...
        channel.send(`@${support_team_role}, @${support_manager_role},  ${member.user} `, embed);
